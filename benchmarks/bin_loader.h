...
struct MmapGuard {
    ...
    bool open(const std::string &path)
    {
        ...
        ptr = mmap(nullptr, len, PROT_READ,
                   MAP_PRIVATE
#ifdef MAP_POPULATE
                       | MAP_POPULATE
#endif
...
inline std::string resolve_bin_path(const std::string &dir_or_file, const std::string &filename)
{
    ...
    std::string dir = (slash != std::string::npos) ? dir_or_file.substr(0, slash + 1) : "./";
    return dir + filename;
}
...
inline AnnDataset load_bin_dataset(const std::string &path, bool eval_only = false,
                                   const std::string &override_base = "", const std::string &override_query = "",
                                   const std::string &override_gt = "", bool train_only = false)
{
    AnnDataset ds;
    ...
    // --- Load base vectors ---
    {
        if (eval_only) {
            ...
            if (fstat(fd, &st) != 0) {
                close(fd);
                throw std::runtime_error("Cannot stat base file: " + base_path);
            }
            ...
            uint8_t hdr[8];
            if (pread(fd, hdr, 8, 0) != 8) {
                close(fd);
                throw std::runtime_error("Cannot read base.bin header: " + base_path);
            }
            close(fd);
            ...
            if (n == 0 || dim == 0 || dim > 10000) {
                throw std::runtime_error("base.bin invalid header: n=" + std::to_string(n) +
                                         " dim=" + std::to_string(dim));
            }
            size_t expected = 8ULL + static_cast<size_t>(n) * dim * sizeof(float);
            if (file_len != expected) {
                throw std::runtime_error("base.bin size mismatch: expected " + std::to_string(expected) +
            ...
            std::memcpy(ds.train.data(), static_cast<const char *>(mm.ptr) + 8,
    ...
    // --- Load query vectors ---
    {
        ...
        if (nq == 0 || dim == 0 || dim > 10000) {
            throw std::runtime_error("query.bin invalid header: nq=" + std::to_string(nq) +
                                     " dim=" + std::to_string(dim));
        }
        if (dim != ds.dim) {
            throw std::runtime_error("query.bin dim (" + std::to_string(dim) + ") != base dim (" +
                                     std::to_string(ds.dim) + ")");
        }

        size_t expected = 8ULL + static_cast<size_t>(nq) * dim * sizeof(float);
        if (mm.len != expected) {
            throw std::runtime_error("query.bin size mismatch: expected " + std::to_string(expected) + " bytes, got " +
                                     std::to_string(mm.len));
        }

        ds.n_test = nq;
        ds.test.resize(static_cast<size_t>(nq) * dim);
        std::memcpy(ds.test.data(), static_cast<const char *>(mm.ptr) + 8,
    ...
    // --- Load ground truth ---
    {
        ...
        if (nq == 0 || k == 0) {
            throw std::runtime_error("gt.bin invalid header: nq=" + std::to_string(nq) + " k=" + std::to_string(k));
        }
        if (nq != ds.n_test) {
            throw std::runtime_error("gt.bin nq (" + std::to_string(nq) + ") != query nq (" +
                                     std::to_string(ds.n_test) + ")");
        }

        size_t expected = 8ULL + static_cast<size_t>(nq) * k * sizeof(int32_t);
        if (mm.len != expected) {
            throw std::runtime_error("gt.bin size mismatch: expected " + std::to_string(expected) + " bytes, got " +
                                     std::to_string(mm.len));
        }

        ds.k_gt = k;
        ds.neighbors.resize(static_cast<size_t>(nq) * k);
        std::memcpy(ds.neighbors.data(), static_cast<const char *>(mm.ptr) + 8,
...
inline DatasetFormat detect_dataset_format(const std::string &path)
{
    ...
    if (stat(path.c_str(), &st) == 0 && S_ISDIR(st.st_mode)) {
        ...
        if (!base_check.empty() && base_check.back() != '/')
            base_check += '/';
        base_check += "base.bin";
