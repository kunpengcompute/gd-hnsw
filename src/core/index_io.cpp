...
std::string idmap_file_for_index(const std::string &base_path) { return base_path + ".idmap"; }

std::string centroids_file_for_index(const std::string &base_path) { return base_path + ".centroids"; }

bool ensure_parent_dir(const std::string &path, std::string &err_out)
{
    std::filesystem::path parent = std::filesystem::path(path).parent_path();
    if (parent.empty()) {
        return true; // no directory component (plain filename in cwd)
    }
    std::error_code ec;
    // create_directories returns false (with ec cleared) when the directory
    // already exists — that is success. Only a set ec is an error.
    std::filesystem::create_directories(parent, ec);
    if (ec) {
        err_out = parent.string() + ": " + ec.message();
        return false;
    }
    return true;
}
...
IdMapLoadStatus load_idmap_file(const std::string &base_path, std::vector<uint32_t> &idmap_out)
{
    ...
    if (hdr.count > static_cast<uint64_t>(INT32_MAX)) {
        fprintf(stderr, "Error: idmap '%s' too large: %lu entries\n", path.c_str(),
                static_cast<unsigned long>(hdr.count));
        fclose(fp);
        return IdMapLoadStatus::Error;
    }
...
{
    if (num_shards == 0) {
        fprintf(stderr, "Error: load_shard_buffers called with num_shards=0\n");
        return false;
    }
    ...
    for (uint32_t s = 0; s < num_shards; s++) {
        ...
        auto result = validate_superblock(shard_bufs[s].data(), static_cast<uint64_t>(file_size));
        if (!result.ok) {
    ...
    for (uint32_t s = 0; s < num_shards; s++) {
        ...
        if (sb->ntotal != expected_ntotal || sb->dim != expected_dim || sb->M != expected_M ||
        ...
        if (sb->global_id_begin != sum_ntotal_local) {
            fprintf(stderr, "Error: shard %u has global_id_begin=%lu, expected %lu\n", s,
                    (unsigned long)sb->global_id_begin, (unsigned long)sum_ntotal_local);
            return false;
        }
