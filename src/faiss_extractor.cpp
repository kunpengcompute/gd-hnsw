...
std::vector<uint32_t> build_cluster_old_gid_of_new(
        const float* vectors,
        uint64_t n,
        uint32_t dim,
        uint32_t num_shards,
    ...
    centroid_index.search(static_cast<faiss::idx_t>(n), vectors, 1,
                          cand_dists.data(), cand_ids.data());
    ...
    for (uint32_t s = 0; s < num_shards; s++) {
        ...
        if (bucket[s].empty()) {
            throw std::runtime_error(
                "k-means produced empty cluster for shard " + std::to_string(s) +
...
std::vector<std::vector<char>> FaissExtractor::extract_sharded(
    ...
    if (num_shards == 0 || num_shards > 64) {
        throw std::runtime_error("num_shards must be in [1, 64]");
    }
    ...
    for (uint32_t s = 0; s < num_shards; s++) {
        ...
        sb.state = static_cast<uint32_t>(ShmState::BUILDING);
        ...
        sb.build_unix_ns = static_cast<uint64_t>(
        ...
        for (uint64_t i = 0; i < n_local; i++) {
            uint64_t new_gid = g_begin + i;
            uint32_t old_gid = (*old_gid_of_new)[static_cast<size_t>(new_gid)];
            std::memcpy(dst_vectors + i * dim,
                        src_vectors + static_cast<uint64_t>(old_gid) * dim,
                        static_cast<size_t>(dim) * sizeof(float));
        }

        // Copy levels
        auto* dst_levels = reinterpret_cast<int32_t*>(
            buf.data() + sb.blocks[BLK_LEVELS].off);
        for (uint64_t i = 0; i < n_local; i++) {
            uint64_t new_gid = g_begin + i;
            uint32_t old_gid = (*old_gid_of_new)[static_cast<size_t>(new_gid)];
        ...
        // Copy local offsets
        std::memcpy(buf.data() + sb.blocks[BLK_OFFSETS].off,
                    local_offsets.data(), local_offsets.size() * sizeof(uint64_t));

        // Copy neighbors with old->new remap
        auto* dst_neighbors = reinterpret_cast<int32_t*>(
            buf.data() + sb.blocks[BLK_NEIGHBORS].off);
        for (uint64_t i = 0; i < n_local; i++) {
            uint64_t new_gid = g_begin + i;
            uint32_t old_gid = (*old_gid_of_new)[static_cast<size_t>(new_gid)];
        ...
        auto* dst_cum = reinterpret_cast<int32_t*>(
            buf.data() + sb.blocks[BLK_CUM_NNEIGHBOR].off);
        for (uint32_t i = 0; i < num_levels; i++) {
            dst_cum[i] = static_cast<int32_t>(hnsw.cum_nneighbor_per_level[i]);
        }

        // Finalize
        sb.state = static_cast<uint32_t>(ShmState::SEALED);
...
std::vector<std::vector<char>> FaissExtractor::build_and_extract_sharded(
        const float* vectors, uint64_t n, uint32_t dim,
        uint32_t num_shards,
        uint32_t M, uint32_t ef_construction, uint32_t ef_search,
        const ShardBuildOptions& opts,
    ...
    auto index = std::make_unique<faiss::IndexHNSWFlat>(
        static_cast<int>(dim), static_cast<int>(M));
    ...
    if (opts.cluster_partition) {
        cluster_old_gid_of_new = build_cluster_old_gid_of_new(
            indexed_vectors, n, dim, num_shards, opts, cluster_shard_sizes, out_centroids);
    ...
    if (opts.cluster_partition && old_gid_of_new_ptr) {
        ...
        auto shard_of = [&](uint32_t new_gid) -> uint32_t {
            ...
            while (lo + 1 < hi) {
                ...
                if (shard_begin[mid] <= new_gid) lo = mid;
                else hi = mid;
            }
            return lo;
        };
        ...
        int layer0_slots = hnsw.cum_nneighbor_per_level.size() >= 2
            ? static_cast<int>(hnsw.cum_nneighbor_per_level[1]) : 0;
        for (uint64_t old_gid = 0; old_gid < n; old_gid++) {
            ...
            int slots = std::min(layer0_slots,
                static_cast<int>(hnsw.offsets[old_gid + 1] - hnsw.offsets[old_gid]));
            for (int j = 0; j < slots; j++) {
                ...
                total_edges++;
                if (nb_shard != my_shard) cross_edges++;
            }
        }
        double cut_ratio = (total_edges > 0)
            ? (100.0 * cross_edges / total_edges) : 0.0;
        printf("[cluster-diag] layer-0 edge cut: %lu / %lu (%.1f%% cross-shard)\n",
               (unsigned long)cross_edges, (unsigned long)total_edges, cut_ratio);
    }

    // Extract shard buffers while index is still alive, then destroy the
    // index immediately to free ~145 GiB before returning.
    auto result = extract_sharded(*index, num_shards, ef_search,
                                  old_gid_of_new_ptr, shard_sizes_ptr);
...
std::vector<uint64_t> FaissExtractor::build_and_extract_sharded_streaming(
    ...
    if (opts.cluster_partition) {
        cluster_old_gid_of_new = build_cluster_old_gid_of_new(
            storage->get_xb(), n, dim, num_shards, opts,
    ...
    uint32_t M_val = static_cast<uint32_t>(hnsw.cum_nneighbor_per_level.size() >= 2
        ? hnsw.cum_nneighbor_per_level[1] / 2 : 0);
    uint32_t num_levels = static_cast<uint32_t>(hnsw.cum_nneighbor_per_level.size());
    ...
    struct ShardRange { uint64_t global_begin; uint64_t count; };
    std::vector<ShardRange> ranges(num_shards);
    {
        uint64_t offset = 0;
        for (uint32_t s = 0; s < num_shards; s++) {
            uint64_t count = shard_sizes_ptr
                ? (*shard_sizes_ptr)[s]
                : (ntotal / num_shards + (s < ntotal % num_shards ? 1 : 0));
    ...
    for (uint32_t s = 0; s < num_shards; s++) {
        ...
        sb.faiss_patch = FAISS_VERSION_PATCH;
#else
        sb.faiss_major = 1; sb.faiss_minor = 14; sb.faiss_patch = 1;
#endif
        sb.metric_type = METRIC_L2;
        sb.storage_idx_bits = 32;
        sb.endianness = ENDIAN_LITTLE;
        ...
        if (!fp) {
            throw std::runtime_error(
                std::string("cannot open '") + path + "' for writing: " + strerror(errno));
        }
        size_t written = fwrite(buf.data(), 1, buf.size(), fp);
        fclose(fp);
        if (written != buf.size()) {
            throw std::runtime_error(
                std::string("short write to '") + path + "'");
        }
