...
ValidationResult validate_superblock(const void* shm_base, uint64_t shm_size) {
    ...
    if (shm_size < SUPERBLOCK_SIZE) {
        return fail("shm region smaller than SuperBlock");
    }

    const auto& sb = *static_cast<const SuperBlockV1*>(shm_base);

    // Magic
    if (sb.magic != SHM_MAGIC) {
    ...
    // State
    if (sb.state > static_cast<uint32_t>(ShmState::ACTIVE)) {
    ...
    // Total size
    if (sb.total_bytes > shm_size) {
        return fail("total_bytes exceeds shm region size");
    }
    ...
    if (sb.state >= static_cast<uint32_t>(ShmState::SEALED)) {
        if (sb.header_crc64 != compute_header_crc64(sb)) {
            return fail("header_crc64 mismatch");
        }
        if (sb.payload_crc64 != compute_payload_crc64(shm_base, sb)) {
    ...
    if (sb.state >= static_cast<uint32_t>(ShmState::SEALED)) {
        const auto* offsets = offsets_ptr(shm_base, sb);
        const auto* levels = levels_ptr(shm_base, sb);
        const auto* cum_nn = cum_nneighbor_ptr(shm_base, sb);
        ...
        const auto* neighbors = neighbors_ptr(shm_base, sb);
        for (uint64_t i = 0; i < sb.neighbors_size; i++) {
            int32_t nid = neighbors[i];
            if (nid != EMPTY_NEIGHBOR &&
                (nid < 0 || static_cast<uint64_t>(nid) >= sb.ntotal)) {
