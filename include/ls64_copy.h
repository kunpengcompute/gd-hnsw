...
//
// For cross-NUMA GD channels where data won't be re-read locally,
...
// ============================================================
// Phase 1: GD → local (read from remote shared memory)
// Uses LDNP (non-temporal load, no cache alloc) + STP (normal store
// to local buffer that will be iterated immediately).
// ============================================================

inline void ls64_copy_from_gd(void *__restrict dst, const void *__restrict src, size_t bytes)
{
    ...
    while (bytes >= 64) {
        __asm__ __volatile__("ldnp q0, q1, [%[s]]       \n"
                             "ldnp q2, q3, [%[s], #32]  \n"
                             "stp  q0, q1, [%[d]]       \n"
                             "stp  q2, q3, [%[d], #32]  \n"
                             :
...
// ============================================================
// Phase 2 & 4: local → GD (write to remote shared memory)
// Uses LDP (normal load from local) + STNP (non-temporal store,
// bypasses cache — avoids RFO on remote NUMA node).
// ============================================================

inline void ls64_copy_to_gd(void *__restrict dst, const void *__restrict src, size_t bytes)
{
    const char *s = static_cast<const char *>(src);
    char *d = static_cast<char *>(dst);
    ...
    while (bytes >= 64) {
        __asm__ __volatile__("ldp  q0, q1, [%[s]]       \n"
                             "ldp  q2, q3, [%[s], #32]  \n"
                             "stnp q0, q1, [%[d]]       \n"
                             "stnp q2, q3, [%[d], #32]  \n"
                             :
