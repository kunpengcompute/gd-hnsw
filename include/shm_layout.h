...
static constexpr uint64_t SHM_MAGIC = 0x484E5357534D4800ULL; // "HNSWSMH\0"
...
// ============================================================
// BlockDesc — describes one contiguous block in shm
// ============================================================

struct BlockDesc {
    uint64_t off;        // byte offset from shm base
...
struct SuperBlockV1 {
    ...
    uint64_t total_bytes;      // total shm region size for this shard
...
static_assert(sizeof(SuperBlockV1) <= SUPERBLOCK_SIZE,
              "SuperBlockV1 must fit in one page");

// ============================================================
// Inline accessors for shm-mapped data
// ============================================================

// Get pointer to a block's data given shm base address
inline const void* block_ptr(const void* shm_base, const SuperBlockV1& sb, BlockIndex idx) {
    return static_cast<const char*>(shm_base) + sb.blocks[idx].off;
}

inline const float* vectors_ptr(const void* shm_base, const SuperBlockV1& sb) {
    return static_cast<const float*>(block_ptr(shm_base, sb, BLK_VECTORS));
}

inline const int32_t* levels_ptr(const void* shm_base, const SuperBlockV1& sb) {
    return static_cast<const int32_t*>(block_ptr(shm_base, sb, BLK_LEVELS));
}

inline const uint64_t* offsets_ptr(const void* shm_base, const SuperBlockV1& sb) {
    return static_cast<const uint64_t*>(block_ptr(shm_base, sb, BLK_OFFSETS));
}

inline const int32_t* neighbors_ptr(const void* shm_base, const SuperBlockV1& sb) {
    return static_cast<const int32_t*>(block_ptr(shm_base, sb, BLK_NEIGHBORS));
}

inline const int32_t* cum_nneighbor_ptr(const void* shm_base, const SuperBlockV1& sb) {
    return static_cast<const int32_t*>(block_ptr(shm_base, sb, BLK_CUM_NNEIGHBOR));
}
...
// Full implementation in shm_layout.cpp
ValidationResult validate_superblock(const void* shm_base, uint64_t shm_size);
...
inline uint64_t align_up(uint64_t offset, uint64_t alignment) {
...
inline uint64_t compute_payload_crc64(const void* shm_base, const SuperBlockV1& sb) {
    uint64_t state = FNV1A64_OFFSET;
    for (uint32_t i = 0; i < BLK_COUNT; i++) {
        const auto& blk = sb.blocks[i];
        const void* ptr = static_cast<const char*>(shm_base) + blk.off;
