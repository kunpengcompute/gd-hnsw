...
inline AnnDataset load_hdf5_dataset(const std::string& path,
    ...
    auto read_float2d = [&](const std::string& name, std::vector<float>& out,
                            uint64_t& nrows, uint32_t& ncols) {
    ...
    auto read_int2d = [&](const std::string& name, std::vector<int32_t>& out,
                          uint64_t& nrows, uint32_t& ncols) {
        H5::DataSet dataset = file.openDataSet(name);
        H5::DataSpace space = dataset.getSpace();
        if (space.getSimpleExtentNdims() != 2) {
    ...
    auto read_float2d_shape = [&](const std::string& name,
                                  uint64_t& nrows, uint32_t& ncols) {
        H5::DataSet dataset = file.openDataSet(name);
        H5::DataSpace space = dataset.getSpace();
        if (space.getSimpleExtentNdims() != 2) {
