...
inline void l2_sqr_batch_4(
    ...
    for (; i < d; i++) {
        float qi = q[i];
        float t0 = qi - v0[i]; dis0 += t0 * t0;
        float t1 = qi - v1[i]; dis1 += t1 * t1;
        float t2 = qi - v2[i]; dis2 += t2 * t2;
        float t3 = qi - v3[i]; dis3 += t3 * t3;
    }
}
